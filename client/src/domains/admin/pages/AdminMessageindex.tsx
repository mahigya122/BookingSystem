...
export default function AdminMessages() {
  ...
  const { messages, sendMessage, bottomRef } = useSupportMessages(activeConv?.id ?? null, 'admin')

  // My presence
  useOnlinePresence()

  // Watch guest's presence
  const { isOnline, lastSeenAt } = useWatchPresence(activeConv?.guest_id ?? null)

  // Typing
  const { otherIsTyping, setTyping } = useTyping(activeConv?.id ?? null, user?.id ?? null)
    ...
    <div className="flex h-screen bg-slate-50 dark:bg-slate-900">
      ...
      <div className="w-72 bg-white dark:bg-slate-800 border-r border-slate-100 dark:border-slate-700 flex flex-col">
        <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-700">
          <p className="font-semibold text-slate-800 dark:text-white text-sm">Support Inbox</p>
          <p className="text-xs text-slate-400 mt-0.5">{conversations.length} conversations</p>
        </div>

        <div className="flex-1 overflow-y-auto">
          {conversations.length === 0 && (
            <p className="text-xs text-slate-400 text-center mt-10">No conversations yet</p>
          )}
          {conversations.map((conv) => (
            <button
              key={conv.id}
              onClick={() => { setActiveConv(conv); setInput('') }}
      ...
      <div className="flex-1 flex flex-col min-h-0">
        {activeConv ? (
              ...
              {messages.map((msg, i) => {
                ...
                return (
                  <div key={msg.id} className={`flex items-end gap-2 ${isMe ? 'justify-end' : 'justify-start'}`}>
                    {/* Guest avatar — only on last message in their group */}
                    {!isMe && (
                      <div className={`w-7 h-7 rounded-full bg-gradient-to-br from-sky-400 to-blue-600 flex items-center justify-center text-white text-xs font-bold shrink-0 ${
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-2">
            ...
            <p className="text-xs text-slate-400">Choose a guest from the left to start replying</p>
          </div>
        )}
      </div>
    </div>
