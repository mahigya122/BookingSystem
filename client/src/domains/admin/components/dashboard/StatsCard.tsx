    ...
    return (
        <div className="group card p-8 flex items-start justify-between relative overflow-hidden hover:border-sky-200 dark:hover:border-sky-800 transition-all duration-300">
            ...
            <div className="flex flex-col gap-2 relative z-10 w-full min-w-0">
                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 group-hover:text-sky-500 transition-colors truncate">{title}</span>
                {isLoading ? (
                    <div className="h-9 w-24 animate-pulse rounded-xl bg-slate-100 dark:bg-slate-800 mt-1" />
                ) : (
                    <span className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">{value}</span>
                )}
            </div>

            <div className={`relative z-10 flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl ${color} text-sky-600 dark:text-sky-400 border border-sky-100/50 dark:border-sky-800/50 shadow-sm group-hover:scale-110 transition-transform duration-300`}>
              {icon}
            </div>
        </div>
    );
