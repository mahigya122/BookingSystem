                            ...
                            {offers.map(offer => (
                                <button 
                                    ...
                                    className={`w-full flex items-center justify-between p-4 rounded-2xl border-2 transition-all group ${
                                        form.offer_ids?.includes(offer.id)
                                ...
                                >
                                    <div className="flex flex-col items-start text-left">
                                        <span className={`text-xs font-black transition-colors ${
                                            form.offer_ids?.includes(offer.id) ? "text-emerald-700 dark:text-emerald-400" : "text-slate-600 dark:text-slate-300"
                                        }`}>
                                            {offer.title || offer.name}
                                        </span>
                                        <span className="text-[10px] font-bold text-emerald-500/80">{(offer.discount_percent ?? offer.discount_pct ?? 0)}% Reduction</span>
                                    </div>
                                    <CheckCircle2 size={18} className={`transition-all ${
                                        form.offer_ids?.includes(offer.id) ? "text-emerald-500 opacity-100 scale-110" : "text-slate-200 dark:text-slate-800 opacity-0 scale-90"
                                    }`} />
                                </button>
                            ))}
                            {offers.length === 0 && (
                        ...
                        <div className="space-y-2 max-h-[280px] overflow-y-auto pr-2 custom-scrollbar">
                            {activities.map(activity => (
                                <button 
                                    ...
                                    className={`w-full flex items-center justify-between p-4 rounded-2xl border-2 transition-all group ${
                                        form.activity_ids?.includes(activity.id)
                                ...
                                >
                                    <div className="flex flex-col items-start text-left">
                                        <span className={`text-xs font-black transition-colors ${
                                            form.activity_ids?.includes(activity.id) ? "text-cyan-700 dark:text-cyan-400" : "text-slate-600 dark:text-slate-300"
                                        }`}>
                                            {activity.name}
                                        </span>
                                    ...
                                    <CheckCircle2 size={18} className={`transition-all ${
                                        form.activity_ids?.includes(activity.id) ? "text-cyan-500 opacity-100 scale-110" : "text-slate-200 dark:text-slate-800 opacity-0 scale-90"
                                    }`} />
                                </button>
                            ))}
                            {activities.length === 0 && (
