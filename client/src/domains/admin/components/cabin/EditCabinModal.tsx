...
import { X, Save, Image as ImageIcon, MapPin, Users, DollarSign, Tag, Sparkles, Loader2 } from "lucide-react";
    ...
    <div className="modal-overlay">
        <div className="modal-content w-full max-w-3xl max-h-[92vh] overflow-hidden flex flex-col shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="px-8 py-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-white dark:bg-slate-900">
                <div>
                    <h2 className="text-2xl font-black tracking-tight text-slate-900 dark:text-white">Edit Cabin</h2>
                    <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Updating: {cabin.name}</p>
                </div>
            ...
            <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-8 space-y-8">
                ...
                <section className="space-y-6">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <h3 className="text-sm font-black uppercase tracking-widest text-slate-900 dark:text-white">General Information</h3>
                    </div>
                ...
                <section className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4 border-t border-slate-100 dark:border-slate-800">
                    <div className="space-y-4">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <h3 className="text-[11px] font-black uppercase tracking-widest text-slate-900 dark:text-white">Active Offers</h3>
                        </div>
                        <div className="grid grid-cols-1 gap-2 bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl max-h-48 overflow-y-auto border border-slate-200 dark:border-slate-800">
                            ...
                            {offers.length === 0 && <p className="text-[10px] text-slate-400 italic py-4 text-center">No offers available.</p>}
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <h3 className="text-[11px] font-black uppercase tracking-widest text-slate-900 dark:text-white">Active Activities</h3>
                        </div>
                        <div className="grid grid-cols-1 gap-2 bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl max-h-48 overflow-y-auto border border-slate-200 dark:border-slate-800">
                            ...
                            {activities.length === 0 && <p className="text-[10px] text-slate-400 italic py-4 text-center">No activities available.</p>}
                        </div>
                    </div>
                </section>
            </form>
