    ...
    if (isSignUp) {
      ...
      signup(
        { email, password },
        {
          ...
          onSuccess: () => {
            setSuccessMessage(
              `Account created successfully! Please check and confirm the email (${email}) you used to sign up. You must confirm this email before you can log in.`,
            );
            setIsSignUp(false);
            setPassword("");
            setConfirmPassword("");
    ...
    <div className="w-full max-w-md">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="space-y-2">
          <label className="ml-0.5 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
            Email Address
          </label>
